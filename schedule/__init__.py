...
from utils.scheduler_metrics import save_next_run_time, get_next_run_time, cron_callback
...
class Job(object):
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def do(self, job_func, cron_callback, *args, **kwargs):
        """
        ...
        self.cron_callback = functools.partial(cron_callback, *args, **kwargs)
        try:
            functools.update_wrapper(self.job_func, job_func)
            functools.update_wrapper(self.cron_callback, cron_callback)
        ...
        jd = get_next_run_time()
        fname = self.job_func.func.__name__
        if fname in jd and int(jd[fname]) > int(datetime.datetime.now().strftime("%Y%m%d%H%M%S")) and int(jd[fname]) < int(self.next_run.strftime("%Y%m%d%H%M%S")):
            self.next_run = datetime.datetime.strptime(jd[fname], '%Y%m%d%H%M%S')
        elif fname not in jd:
            save_next_run_time(fname,self.next_run.strftime("%Y%m%d%H%M%S"))
    ...
    def run(self):
        ...
        try :
            ...
            self.cron_callback(error)
        self.last_run = datetime.datetime.now()
        self._schedule_next_run()
        save_next_run_time(self.job_func.func.__name__, self.next_run.strftime("%Y%m%d%H%M%S"))
        return ret
